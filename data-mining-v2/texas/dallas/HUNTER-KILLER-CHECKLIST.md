# Dallas County, TX - Hunter-Killer v2.0 Research Checklist
...
```
site:.gov "sheriff" "towing" OR "impound" "release procedure" filetype:pdf Dallas TX
```
...
```
site:.gov "towing" "fee schedule" "after hours" OR "holiday" Dallas Dallas
```
...
```
site:.gov "Dallas" TX "vehicle storage facility" "notarized" OR "power of attorney"
```

### IMPOUND - Fee Schedule
```
site:.gov "towing ordinance" "maximum fee" Dallas TX
```
...
```
site:.gov "inmate handbook" "release processing" "shift change" Dallas TX 2025
```
...
```
site:.gov "magistrate" "standing order" "DUI" OR "DWI" "release" Dallas TX 2025
```
...
```
site:.gov "bail schedule" "misdemeanor" "DUI" OR "DWI" 2025 filetype:pdf Dallas TX
```
...
```
site:.gov "bail schedule" "felony" "DUI" OR "DWI" 2025 filetype:pdf Dallas TX
```
...
### BAIL - PR Bond Rules
```
site:.gov "personal recognizance" OR "PR bond" "DUI" eligibility Dallas TX 2025
```

### BAIL - Bonding Office Hours
```
site:.gov "Dallas" sheriff "bonding" hours location 2025
```
EXTRACT:
- Address
...
### DMV - Discovery Fees
```
site:.gov "open records" "fee schedule" "body cam" OR "video" Dallas police 2025
```
...
```
site:.gov "ALR" "hearing location" "zoom" OR "telephonic" TX Dallas 2025
```
EXTRACT:
...
```
site:.gov "probation" "approved providers" "interlock" Dallas TX
```
...
```
site:.gov "interlock" "removal order" OR "verification of removal" form Dallas TX
```
EXTRACT:
...
```
site:.gov "SCRAM" OR "continuous alcohol monitor" "approved" provider Dallas TX
```
EXTRACT:
For each provider:
- Name
...
```
site:.gov "SCRAM violation" "probation revocation" Dallas TX
```
EXTRACT:
...
```
site:.gov "pre-trial intervention" OR "diversion" "contract" OR "agreement" filetype:pdf Dallas TX
```
...
### COURT - Local Rules
```
site:.gov "local rules of practice" "criminal" "cell phone" Dallas clerk
```
...
### COURT - Weekend Jail
```
site:.gov "weekend jail" OR "periodic imprisonment" "application" Dallas sheriff
```
...
### COURT - Parking Hack
```
site:.gov "Dallas" "jury service" parking map
"Dallas" TX courthouse parking juror lot free shuttle
```
