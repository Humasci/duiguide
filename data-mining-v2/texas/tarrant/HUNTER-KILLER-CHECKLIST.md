# Tarrant County, TX - Hunter-Killer v2.0 Research Checklist
...
```
site:.gov "sheriff" "towing" OR "impound" "release procedure" filetype:pdf Tarrant TX
```
...
```
site:.gov "towing" "fee schedule" "after hours" OR "holiday" Fort Worth Tarrant
```
...
```
site:.gov "Tarrant" TX "vehicle storage facility" "notarized" OR "power of attorney"
```

### IMPOUND - Fee Schedule
```
site:.gov "towing ordinance" "maximum fee" Fort Worth TX
```
...
```
site:.gov "inmate handbook" "release processing" "shift change" Tarrant TX 2025
```
...
```
site:.gov "magistrate" "standing order" "DUI" OR "DWI" "release" Tarrant TX 2025
```
...
```
site:.gov "bail schedule" "misdemeanor" "DUI" OR "DWI" 2025 filetype:pdf Tarrant TX
```
...
```
site:.gov "bail schedule" "felony" "DUI" OR "DWI" 2025 filetype:pdf Tarrant TX
```
...
### BAIL - PR Bond Rules
```
site:.gov "personal recognizance" OR "PR bond" "DUI" eligibility Tarrant TX 2025
```

### BAIL - Bonding Office Hours
```
site:.gov "Tarrant" sheriff "bonding" hours location 2025
```
EXTRACT:
- Address
...
### DMV - Discovery Fees
```
site:.gov "open records" "fee schedule" "body cam" OR "video" Fort Worth police 2025
```
...
```
site:.gov "ALR" "hearing location" "zoom" OR "telephonic" TX Tarrant 2025
```
EXTRACT:
...
```
site:.gov "probation" "approved providers" "interlock" Tarrant TX
```
...
```
site:.gov "interlock" "removal order" OR "verification of removal" form Tarrant TX
```
EXTRACT:
...
```
site:.gov "SCRAM" OR "continuous alcohol monitor" "approved" provider Tarrant TX
```
EXTRACT:
For each provider:
- Name
...
```
site:.gov "SCRAM violation" "probation revocation" Tarrant TX
```
EXTRACT:
...
```
site:.gov "pre-trial intervention" OR "diversion" "contract" OR "agreement" filetype:pdf Tarrant TX
```
...
### COURT - Local Rules
```
site:.gov "local rules of practice" "criminal" "cell phone" Tarrant clerk
```
...
### COURT - Weekend Jail
```
site:.gov "weekend jail" OR "periodic imprisonment" "application" Tarrant sheriff
```
...
### COURT - Parking Hack
```
site:.gov "Tarrant" "jury service" parking map
"Tarrant" TX courthouse parking juror lot free shuttle
```
