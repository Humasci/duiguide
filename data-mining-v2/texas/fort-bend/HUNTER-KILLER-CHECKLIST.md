# Fort Bend County, TX - Hunter-Killer v2.0 Research Checklist
...
```
site:.gov "sheriff" "towing" OR "impound" "release procedure" filetype:pdf Fort Bend TX
```
...
```
site:.gov "towing" "fee schedule" "after hours" OR "holiday" Sugar Land Fort Bend
```
...
```
site:.gov "Fort Bend" TX "vehicle storage facility" "notarized" OR "power of attorney"
```

### IMPOUND - Fee Schedule
```
site:.gov "towing ordinance" "maximum fee" Sugar Land TX
```
...
```
site:.gov "probation" "approved providers" "interlock" Fort Bend TX
```
...
```
site:.gov "interlock" "removal order" OR "verification of removal" form Fort Bend TX
```
EXTRACT:
...
```
site:.gov "SCRAM" OR "continuous alcohol monitor" "approved" provider Fort Bend TX
```
EXTRACT:
For each provider:
- Name
...
```
site:.gov "SCRAM violation" "probation revocation" Fort Bend TX
```
EXTRACT:
...
```
site:.gov "pre-trial intervention" OR "diversion" "contract" OR "agreement" filetype:pdf Fort Bend TX
```
...
### COURT - Local Rules
```
site:.gov "local rules of practice" "criminal" "cell phone" Fort Bend clerk
```
...
### COURT - Weekend Jail
```
site:.gov "weekend jail" OR "periodic imprisonment" "application" Fort Bend sheriff
```
...
### COURT - Parking Hack
```
site:.gov "Fort Bend" "jury service" parking map
"Fort Bend" TX courthouse parking juror lot free shuttle
```
