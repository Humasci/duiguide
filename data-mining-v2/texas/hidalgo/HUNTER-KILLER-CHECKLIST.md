# Hidalgo County, TX - Hunter-Killer v2.0 Research Checklist
...
```
site:.gov "sheriff" "towing" OR "impound" "release procedure" filetype:pdf Hidalgo TX
```
...
```
site:.gov "towing" "fee schedule" "after hours" OR "holiday" McAllen Hidalgo
```
...
```
site:.gov "Hidalgo" TX "vehicle storage facility" "notarized" OR "power of attorney"
```

### IMPOUND - Fee Schedule
```
site:.gov "towing ordinance" "maximum fee" McAllen TX
```
...
```
site:.gov "inmate handbook" "release processing" "shift change" Hidalgo TX 2025
```
...
```
site:.gov "magistrate" "standing order" "DUI" OR "DWI" "release" Hidalgo TX 2025
```
...
```
site:.gov "bail schedule" "misdemeanor" "DUI" OR "DWI" 2025 filetype:pdf Hidalgo TX
```
...
```
site:.gov "bail schedule" "felony" "DUI" OR "DWI" 2025 filetype:pdf Hidalgo TX
```
...
### BAIL - PR Bond Rules
```
site:.gov "personal recognizance" OR "PR bond" "DUI" eligibility Hidalgo TX 2025
```

### BAIL - Bonding Office Hours
```
site:.gov "Hidalgo" sheriff "bonding" hours location 2025
```
EXTRACT:
- Address
...
### DMV - Discovery Fees
```
site:.gov "open records" "fee schedule" "body cam" OR "video" McAllen police 2025
```
...
```
site:.gov "ALR" "hearing location" "zoom" OR "telephonic" TX Hidalgo 2025
```
EXTRACT:
...
```
site:.gov "probation" "approved providers" "interlock" Hidalgo TX
```
...
```
site:.gov "interlock" "removal order" OR "verification of removal" form Hidalgo TX
```
EXTRACT:
...
```
site:.gov "SCRAM" OR "continuous alcohol monitor" "approved" provider Hidalgo TX
```
EXTRACT:
For each provider:
- Name
...
```
site:.gov "SCRAM violation" "probation revocation" Hidalgo TX
```
EXTRACT:
...
```
site:.gov "pre-trial intervention" OR "diversion" "contract" OR "agreement" filetype:pdf Hidalgo TX
```
...
### COURT - Local Rules
```
site:.gov "local rules of practice" "criminal" "cell phone" Hidalgo clerk
```
...
### COURT - Weekend Jail
```
site:.gov "weekend jail" OR "periodic imprisonment" "application" Hidalgo sheriff
```
...
### COURT - Parking Hack
```
site:.gov "Hidalgo" "jury service" parking map
"Hidalgo" TX courthouse parking juror lot free shuttle
```
