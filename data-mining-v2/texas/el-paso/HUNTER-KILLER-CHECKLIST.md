# El Paso County, TX - Hunter-Killer v2.0 Research Checklist
...
```
site:.gov "sheriff" "towing" OR "impound" "release procedure" filetype:pdf El Paso TX
```
...
```
site:.gov "towing" "fee schedule" "after hours" OR "holiday" El Paso El Paso
```
...
```
site:.gov "El Paso" TX "vehicle storage facility" "notarized" OR "power of attorney"
```

### IMPOUND - Fee Schedule
```
site:.gov "towing ordinance" "maximum fee" El Paso TX
```
...
```
site:.gov "inmate handbook" "release processing" "shift change" El Paso TX 2025
```
...
```
site:.gov "magistrate" "standing order" "DUI" OR "DWI" "release" El Paso TX 2025
```
...
```
site:.gov "bail schedule" "misdemeanor" "DUI" OR "DWI" 2025 filetype:pdf El Paso TX
```
...
```
site:.gov "bail schedule" "felony" "DUI" OR "DWI" 2025 filetype:pdf El Paso TX
```
...
### BAIL - PR Bond Rules
```
site:.gov "personal recognizance" OR "PR bond" "DUI" eligibility El Paso TX 2025
```

### BAIL - Bonding Office Hours
```
site:.gov "El Paso" sheriff "bonding" hours location 2025
```
EXTRACT:
- Address
...
### DMV - Discovery Fees
```
site:.gov "open records" "fee schedule" "body cam" OR "video" El Paso police 2025
```
...
```
site:.gov "ALR" "hearing location" "zoom" OR "telephonic" TX El Paso 2025
```
EXTRACT:
...
```
site:.gov "probation" "approved providers" "interlock" El Paso TX
```
...
```
site:.gov "interlock" "removal order" OR "verification of removal" form El Paso TX
```
EXTRACT:
...
```
site:.gov "SCRAM" OR "continuous alcohol monitor" "approved" provider El Paso TX
```
EXTRACT:
For each provider:
- Name
...
```
site:.gov "SCRAM violation" "probation revocation" El Paso TX
```
EXTRACT:
...
```
site:.gov "pre-trial intervention" OR "diversion" "contract" OR "agreement" filetype:pdf El Paso TX
```
...
### COURT - Local Rules
```
site:.gov "local rules of practice" "criminal" "cell phone" El Paso clerk
```
...
### COURT - Weekend Jail
```
site:.gov "weekend jail" OR "periodic imprisonment" "application" El Paso sheriff
```
...
### COURT - Parking Hack
```
site:.gov "El Paso" "jury service" parking map
"El Paso" TX courthouse parking juror lot free shuttle
```
