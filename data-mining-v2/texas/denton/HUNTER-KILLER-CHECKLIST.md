# Denton County, TX - Hunter-Killer v2.0 Research Checklist
...
```
site:.gov "sheriff" "towing" OR "impound" "release procedure" filetype:pdf Denton TX
```
...
```
site:.gov "towing" "fee schedule" "after hours" OR "holiday" Denton Denton
```
...
```
site:.gov "Denton" TX "vehicle storage facility" "notarized" OR "power of attorney"
```

### IMPOUND - Fee Schedule
```
site:.gov "towing ordinance" "maximum fee" Denton TX
```
...
```
site:.gov "inmate handbook" "release processing" "shift change" Denton TX 2025
```
...
```
site:.gov "magistrate" "standing order" "DUI" OR "DWI" "release" Denton TX 2025
```
...
```
site:.gov "bail schedule" "misdemeanor" "DUI" OR "DWI" 2025 filetype:pdf Denton TX
```
...
```
site:.gov "bail schedule" "felony" "DUI" OR "DWI" 2025 filetype:pdf Denton TX
```
...
### BAIL - PR Bond Rules
```
site:.gov "personal recognizance" OR "PR bond" "DUI" eligibility Denton TX 2025
```

### BAIL - Bonding Office Hours
```
site:.gov "Denton" sheriff "bonding" hours location 2025
```
EXTRACT:
- Address
...
### DMV - Discovery Fees
```
site:.gov "open records" "fee schedule" "body cam" OR "video" Denton police 2025
```
...
```
site:.gov "ALR" "hearing location" "zoom" OR "telephonic" TX Denton 2025
```
EXTRACT:
...
```
site:.gov "probation" "approved providers" "interlock" Denton TX
```
...
```
site:.gov "interlock" "removal order" OR "verification of removal" form Denton TX
```
EXTRACT:
...
```
site:.gov "SCRAM" OR "continuous alcohol monitor" "approved" provider Denton TX
```
EXTRACT:
For each provider:
- Name
...
```
site:.gov "SCRAM violation" "probation revocation" Denton TX
```
EXTRACT:
...
```
site:.gov "pre-trial intervention" OR "diversion" "contract" OR "agreement" filetype:pdf Denton TX
```
...
### COURT - Local Rules
```
site:.gov "local rules of practice" "criminal" "cell phone" Denton clerk
```
...
### COURT - Weekend Jail
```
site:.gov "weekend jail" OR "periodic imprisonment" "application" Denton sheriff
```
...
### COURT - Parking Hack
```
site:.gov "Denton" "jury service" parking map
"Denton" TX courthouse parking juror lot free shuttle
```
